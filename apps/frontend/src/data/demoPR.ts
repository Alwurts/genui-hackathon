import type { Diff, PR } from "@/lib/review/types";

// Hand-crafted demo PR designed so the three preset lenses pick different
// files as #1:
//   Money lens     → src/billing/payment.ts
//   Architecture   → src/cart/pricing.ts (signature hunk)
//   Tests/Quality  → tests/cart.test.ts
//
// Used as the boot fallback when GITHUB_DEMO_PR=cached://demo or whenever a
// real PR fetch fails on stage.
      ...
      hunks: [
        {
          ...
          lines: [
            ...
            { type: "+", content: "}" },
            { type: "+", content: "" },
            { type: "+", content: "export async function lookupCoupon(code: string): Promise<Coupon | null> {" },
            { type: "+", content: "  const row = await db.coupon.findUnique({ where: { code } });" },
            { type: "+", content: "  if (!row) return null;" },
            { type: "+", content: "  if (new Date(row.expiresAt) < new Date()) return null;" },
            { type: "+", content: "  return row;" },
            { type: "+", content: "}" },
          ],
        },
      ],
      ...
      hunks: [
        {
          ...
          lines: [
            ...
            { type: "-", content: "  });" },
            { type: "-", content: "" },
            { type: "-", content: "  it('handles 100% off coupons without going negative', () => {" },
            { type: "-", content: "    const r = priceCart([{ price: 50, qty: 1 }], FULL_OFF);" },
            { type: "-", content: "    expect(r.total).toBeGreaterThanOrEqual(0);" },
            { type: "-", content: "  });" },
            { type: " ", content: "});" },
          ],
        },
